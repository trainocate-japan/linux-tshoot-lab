...
- ストレージの容量、ネットワークの状態も確認しておきましょう。`df` `ip` コマンドなど、これまでの研修で学んだコマンドが使えるはずです
...
- ホームディレクトリの`init.sh` を実行してください。実行の方法は何でも構いません。スクリプトの実行方法は「実務で使えるLinux 入門編」で取り上げていますので、思い出しましょう

...
- トラブルシューティング中には、該当のサーバーにログインできなかったりサーバーからの応答がない、という、通常の演習とは異なる挙動が見られることがあります。講師に相談する際は、その事象がトラブルによるものなのか、それとも演習環境自体の不具合なのかを説明できるようにしましょう
### 演習環境へのログイン方法
...
- セキュリティの都合上、このマシンからはインターネットには繋がりません。ですのでツールのインストールは出来ないのでご注意ください
#### 4) アプリケーションサーバー（app）
...
### 3. workspaceへのログイン
- appサーバーやdbサーバーにログインするには、workspaceを介する必要があります。以下のコマンドで、workspaceにログインしましょう
```
ssh user1@workspace
```
...
- 各ホストでの正常性確認が終わったら、ベースマシにに戻っておいてください（コマンドプロンプトを見てベースマシンであることを認識してください）
...
- LXCはLinux Container という機能で、Linuxでのコンテナ型仮想化の仕組みです。古くから軽量VMとして利用されてきました
  - ちなみにコンテナソリューションとして有名なDockerは、登場した当初はLXCのラッパーでした
...
- Netdataは、インターネットからの直アクセスになるためユーザー認証を設けています。ちなみにポート番号も19999から9999へ変更しています
